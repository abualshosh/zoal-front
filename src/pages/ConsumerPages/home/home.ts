...
public selectedCard:Card=new Card("","","","",false);
    ...
    found:boolean=false;
  constructor(private cardIO: CardIO,private formBuilder: FormBuilder ,public modalCtrl:ModalController,public navCtrl: NavController,public user:UserProvider,public storage:Storage,public alertCtrl:AlertController,public toastCtrl: ToastController) {
...
if(mon.length==1){
   ...
   new Card(dat.CARDNAME,dat.PAN,"XXXXXXXXXX"+dat.PAN.substring(10),date.getFullYear().toString().substring(2,4)+mon,false)
);
          this.storage.set('cards',this.cards);
              this.found=false;
        }
...
editCard(){
     let modal = this.modalCtrl.create('AddCardModelPage' , {"card":this.selectedCard}, { cssClass: 'inset-modal-box' });
...
AddCardModel(){
 let modal = this.modalCtrl.create('AddCardModelPage' ,null, { cssClass: 'inset-modal-box' });
  modal.onDidDismiss(data => {
  this.storage.get('cards').then((val) => {
