...
dat.consumerIdentifier=this.consumerIdentifier;
    ...
    this.GetServicesProvider.load(this.todo.value,'gmpp/getLastTransactions').then(data => {
     this.bal = data;
      console.log(data)
    ...
    var datas =[
      {"tital":"Status","desc":data.responseMessage},
       ...
       let modal = this.modalCtrl.create('LastTransactionsModPage', {"data":data.transactions} ,{cssClass:'inset-modal'} );
     modal.present();
     this.todo.reset();
